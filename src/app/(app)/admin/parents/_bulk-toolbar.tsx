...
import {
  bulkPermanentlyDeleteParents,
  bulkRestoreParents,
  bulkSetParentsArchived,
  bulkSoftDeleteParents,
} from "./_actions";

type View = "active" | "archived" | "deleted";

export function ParentsBulkToolbar({ view }: { view: View }) {
  ...
  function onArchive() {
    startTransition(async () => announce(await bulkSetParentsArchived(ids, true)));
  }
  function onUnarchive() {
    startTransition(async () => announce(await bulkSetParentsArchived(ids, false)));
  }
  function onSoftDelete() {
    startTransition(async () => announce(await bulkSoftDeleteParents(ids)));
  }
  function onRestore() {
    startTransition(async () => announce(await bulkRestoreParents(ids)));
  }
  function onPurge() {
    startTransition(async () => {
      const r = await bulkPermanentlyDeleteParents(ids);
      announce(r);
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2">
          {view === "active" ? (
            <>
              <BulkBtn onClick={onArchive} icon={EyeOff} label={t("bulkArchive")} pending={pending} />
              <BulkBtn onClick={onSoftDelete} icon={Trash2} label={t("bulkDelete")} pending={pending} danger />
            </>
          ) : null}

          {view === "archived" ? (
            <>
              <BulkBtn onClick={onUnarchive} icon={ArchiveRestore} label={t("bulkUnarchive")} pending={pending} />
              <BulkBtn onClick={onSoftDelete} icon={Trash2} label={t("bulkDelete")} pending={pending} danger />
            </>
          ) : null}
          ...
          {view === "deleted" ? (
            <>
              <BulkBtn onClick={onRestore} icon={RotateCcw} label={t("bulkRestore")} pending={pending} />
              <BulkBtn
