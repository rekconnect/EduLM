...
export default async function ClassDetailPage({
  ...
  return withTenantSession(async (user) => {
    ...
    const klass = await db.class.findUnique({
      where: { id },
      include: {
        academicYear: { select: { label: true, id: true, startDate: true } },
        enrollments: {
          include: {
    ...
    // Candidates = students who:
    //   1. Are ENROLLED or PROSPECT
    //   2. Don't already have an enrollment for THIS class's year (existing rule)
    //   3. Don't already have an enrollment for any FUTURE year — those kids
    //      are "committed" elsewhere (e.g. accepted for next year). They
    //      shouldn't appear when enrolling into the current year.
          ...
          enrollments: {
            some: {
              OR: [
                ...
                {
                  academicYear: {
                    startDate: { gt: klass.academicYear.startDate },
                  },
                },
              ],
            },
