...
import { cancelDraftApplication } from "../../../applications/_actions";

/**
 * "Annuler le dossier" — DRAFT-only delete button rendered inside the
 * 10-tab dossier shell at the bottom of the page. Confirmation modal
 * matches the danger-zone pattern used elsewhere (Settings → Delete
 * tenant, /admin/years per-row delete, etc.).
 *
 * Hard-deletes the Application row via `cancelDraftApplication`. The
 * server action is DRAFT-only — we hide the trigger for any other
 * status, so the action's `not-draft` error is just a defensive guard.
 *
 * SUBMITTED applications need a "Retirer" (withdraw) flow that sets
 * status = WITHDRAWN without losing the data. Not in scope here;
 * follow-up task.
 */
